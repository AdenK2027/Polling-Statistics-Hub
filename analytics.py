...
def get_field_data(data):
    ...
    for person in data:
        ...
        result['total'] += 1
        if 'policy' in person.field and person.field.strip() != 'policy':
...
def get_field_party_data(data):
    ...
    try:
        ...
        for party in parties:
            result['policy'][party] = {}
            for response in responses:
                result['policy'][party][response] = 0
            result['policy'][party]['total'] = 0
    ...
    for person in data:
        ...
        if party in parties:
            ...
            if 'policy' in person.field and person.field.strip() != 'policy':
                result['policy'][party][person.response] += 1
                result['policy'][party]['total'] += 1
